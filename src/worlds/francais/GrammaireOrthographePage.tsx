...
export default function GrammaireOrthographePage() {
  const [exercices, setExercices] = useState<Exercice[]>([]);

  useEffect(() => {
    const grammaireList = getExercices("grammaire-orthographe");
    setExercices(grammaireList);
  }, []);
      ...
      <div className="relative z-10 flex flex-col items-center justify-start w-full px-6 py-12 md:px-10">
        <header className="w-full max-w-6xl text-center mb-8">
          ...
          <p className="mt-2 text-loma-violet dark:text-loma-beige/80">
            Accords, conjugaisons et orthographe en douceur.
          </p>
        </header>

        <main className="w-full max-w-6xl">
          <div className="rounded-3xl bg-white/90 dark:bg-loma-brun/60 backdrop-blur-md shadow-loma border border-loma-vert/30 p-6 md:p-8">
            <ExerciseEngine domaine="grammaire_orthographe" exercices={exercices} />
          </div>
          ...
          <div className="mt-8 flex justify-center">
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-[1rem] px-6 py-3 
                         bg-loma-vert/90 hover:bg-loma-vert 
